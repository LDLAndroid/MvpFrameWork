    ...
    public void show(){
        if(mActivity!=null){
            ...
            if(mToastLayout==null){//判断是否已经添加进母VIEW里，没有则添加进去
                ...
                mActivity.addContentView(mToast,new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, CommonUtils.dip2px(mActivity,60)));
            }else{//如果有，则直接取出
            ...
            if(mToastLayout==null){
                ...
                mView.addView(mToast,new RelativeLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, CommonUtils.dip2px(mView.getContext(),60)));
            }else{
                mToast = (ToastLayout) mToastLayout.getParent();
            }
            mToast.setContent(text);
